...
def partial_icosahedron():
	...
	vertices = np.array([[phi, 1.0, 0.0], [phi, -1.0, 0.0], [-phi, -1.0, 0.0], [-phi, 1.0, 0.0], [1.0, 0.0, phi], [-1.0, 0.0, phi], [-1.0, 0.0, -phi], [1.0, 0.0, -phi], [0.0, phi, 1.0], [0.0, phi, -1.0], [0.0, -phi, -1.0], [0.0, -phi, 1.0]])
	vertices = vertices / np.linalg.norm(vertices, axis=1, ord=2).reshape(vertices.shape[0], 1)
	faces = np.array([[5, 4, 8], [4, 5, 11], [8, 4, 0], [4, 1, 0], [4, 11, 1], [11, 10, 1], [11, 2, 10], [5, 2, 11], [1, 10, 7], [0, 1, 7]])
